# SAR-402 Canonical Public Demo Receipt — v2 — PREP (not published)

**Generated (UTC):** 2026-06-23T23:41:56.415646+00:00  
**Artifact id:** `sar402-canonical-public-demo-v2-20260623T234156Z`  
**Proposed payload:** `sar402-canonical-public-demo-v2-20260623T234156Z.payload.json`  
...
## 0. Why v2 (supersedes v1)

The v1 prepared payload (`sha256:ecbcd91bc7dbd847f7cab1dbe4605878cbed499d7726c1db1acc81e3e6e8b177`) was POSTed to production **before the service restart**, while the live service was still running the old code. The POST succeeded but the live service ignored the `receipt_context=public_demo` query param and stored the ledger record as **`receipt_context: real_task`**. v1 is therefore **not** the canonical `public_demo` receipt and must not be reused or reposted.

After restart, production correctly enforces the constrained context: an invalid value now returns HTTP 422 `{"detail":"invalid receipt_context: must be one of real_task, public_demo"}`. This v2 payload has a **different digest** from v1 and is the canonical artifact to publish as `public_demo`.
...
curl -sS -X POST \
  ...
  --data @sar402-canonical-public-demo-v2-20260623T234156Z.payload.json
...
```bash
curl -sS 'https://defaultverifier.com/v1/attest/receipt/sha256%3A91e2ae85f03c7a8e7df10e8862895b99456cb13abc50b4e23ba84f1c15b3b8c9'
# expect a record with receipt_type=sar_402_settlement, receipt_context=public_demo
```

Live lookup path: `/v1/attest/receipt/sha256%3A91e2ae85f03c7a8e7df10e8862895b99456cb13abc50b4e23ba84f1c15b3b8c9`

## 7. SAR Explorer URL pattern (after publication)

```text
https://sarexplorer.com/?receipt_id=sha256%3A91e2ae85f03c7a8e7df10e8862895b99456cb13abc50b4e23ba84f1c15b3b8c9
```
