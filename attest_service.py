...
def get_agent_summary(agent_id: str, limit: int | None = Query(DEFAULT_LIMIT), include_anonymous: bool = False):
    ...
    # badge_url/badge_markdown are retained past the D1 terminal date deliberately:
    # they are actively read by the deployed sar-explorer frontend (explorer.html,
    # start.html) with evidenced live production traffic, and the badge/link targets
    # they point to are already non-scoring, facts-only surfaces (settlement-witness
    # demolition work, morpheus:state/STATE.md). Removing them is a breaking API
    # change requiring its own evidenced migration plan, out of scope here.
